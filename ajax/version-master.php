...
switch ($type) {
	...
	case 'searchfilter': {
			...
			if (strlen($searchcriteria) > 0 || strlen($s_productgroup) > 0) {
				switch ($orderby) {
					case 'slno':
						$orderbyfield = 'ssm_versions.slno';
						break;
					case 'productgroup':
						$orderbyfield = 'ssm_versions.productgroup';
						break;
					case 'productname':
						$orderbyfield = 'ssm_products.productname';
						break;
					case 'productversion':
						$orderbyfield = 'ssm_versions.productversion';
						break;
					case 'releasedate':
						$orderbyfield = 'ssm_versions.releasedate';
						break;
				}
				switch ($selection) {
					case 'slno':
						$textfield = "ssm_versions.slno LIKE '%" . $searchcriteria . "%'";
						break;
					case 'productgroup':
						$textfield = "ssm_versions.productgroup LIKE '%" . $s_productgroup . "%'";
						break;
					case 'productname':
						$textfield = "ssm_products.productname LIKE '%" . $searchcriteria . "%'";
						break;
					case 'releasedate':
						$textfield = "ssm_versions.releasedate LIKE '%" . changedateformat($searchcriteria) . "%'";
						break;
				}
