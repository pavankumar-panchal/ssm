...
if (isset($_POST['errorreportgrid'])) {
    ...
    // Check if $_POST["check"] is set and is an array
    if (isset($_POST["check"]) && is_array($_POST["check"])) {
        $i_n = 0;
        foreach ($_POST["check"] as $bugloadnos) {
            $i_n++;
            $color = ($i_n % 2 == 0) ? "#dbe5f1" : "#ffffff";
        ...
        // Handle the case when $_POST["check"] is not set or not an array
        $grid .= '<tr><td colspan="10">No data available.</td></tr>';
    }
...
$localtime = datetimelocal('His');
$filebasename = "S_BRp" . $localdate . "-" . $localtime . ".xls";
$filepath = $_SERVER['DOCUMENT_ROOT'] . '/mywork/ssm/filecreated/' . $filebasename;
$downloadlink = 'http://' . $_SERVER['HTTP_HOST'] . '/mywork/ssm/filecreated/' . $filebasename;
